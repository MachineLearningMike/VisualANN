...
def run(data):
    ...
    CS00 = ax.contour(X, Y, outputs[0][0,::], clabels0, linewidths=1, linestyles='dashed', colors='blue')
    # ax.clabel(CS00, inline=True, fontsize=6, fmt="%1.1f")
    CS01 = ax.contour(X, Y, outputs[0][1,::], clabels0, linewidths=1, linestyles='dashed', colors='green')
    # ax.clabel(CS01, inline=True, fontsize=6, fmt="%1.1f")
    CS02 = ax.contour(X, Y, outputs[0][2,::], clabels0, linewidths=1, linestyles='dashed', colors='pink')
    CS03 = ax.contour(X, Y, outputs[0][3,::], clabels0, linewidths=1, linestyles='dashed', colors='cyan')
