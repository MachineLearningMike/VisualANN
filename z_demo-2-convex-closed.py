...
def data_gen():
    for cnt in counts: # itertools.count():
        t0 = 2 * np.pi * cnt / 2880
        t1 = t0 + np.pi/2 * 1.5     # Look ------------------
        t2 = t1 + np.pi/2 * 1.5     # Look ------------------
        t3 = t2 + np.pi/2
...
def run(data):
    # update the data
    a00, a01, a10, a11, a20, a21, _, _ = data
    a02 = -1; a12 = -1; a22 = -1
    nn.SetWeights([
        np.array(((a00, a01, a02), (a10, a11, a12), (a20, a21, a22))), 
        np.array(((1., 1., 1., 0.),))
        ])
    ...
    .format(dense0.activation.name, a00, a01, a02, dense0.activation.name, a10, a11, a12, dense0.activation.name, a20, a21, a22), fontsize=10)
