...
nContours = 7
step = 0.4
min = int(dense0.activation.eval(limL) - 1.) - step
max = int(dense0.activation.eval(limH) + 1.) + step
...
Z2 = sigmoid(X - Y -.5)
Z3 = Z1 + Z2
Z4 = np.maximum(0., Z3 - .7)

draw_contour(axes[0,0], X, Y, Z1, "Z1 = sigmoid(Y - X -.5)")
draw_contour(axes[1,0], X, Y, Z2, "Z2 = sigmoid(X - Y -.5)")
draw_contour(axes[0,1], X, Y, Z3, "Z3 = Z1 + Z2")
draw_contour(axes[1,1], X, Y, Z4, "Z4 = Relu( Z3 - .7)")
